...
# Часто встречается: "11 декабря 2025, 10:28"
DATE_RE = re.compile(r"(\d{1,2})\s+([А-Яа-яёЁ]+)\s+(\d{4}),\s*(\d{1,2}):(\d{2})")
RU_MONTHS = {
    "января": 1, "февраля": 2, "марта": 3, "апреля": 4, "мая": 5, "июня": 6,
    "июля": 7, "августа": 8, "сентября": 9, "октября": 10, "ноября": 11, "декабря": 12,
}
LOCAL_TZ = timezone(timedelta(hours=8))  # Иркутск
...
def parse_article(article_url, school_name):
    ...
    # Парсим дату публикации из текста
    pub_dt = None
...
def main():
    # Формат schools.txt: "СОШ №76|https://school76.gosuslugi.ru/roditelyam-i-uchenikam/novosti/"
    schools = []
